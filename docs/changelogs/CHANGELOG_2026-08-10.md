...
## 词库职责分家 + 架构 v3（2026-08-10）

- **词库分家**：词库构建全部移交 taishen-dict 独立管线（构建+校验+VERSION.json 版本清单+sync_to_ime.py hash 对账同步）；本仓库只消费
- **归档**：8 个历史词库构建脚本 → tools/archive/（勿运行）；raw_dict.txt → resources/archive/；package.ps1 第 1 步改词库校验
- **架构 v3**：ARCHITECT.md 重写（v2 归档 architecture-v2.md）——双仓库分工、引擎 21 模块、51 FFI、平台 TSF+UI 控件库、词库 P1→P5 分层、简繁双表

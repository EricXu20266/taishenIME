...
- `domain_words`：169,629 简体（含繁体转简版——简体模式也能打出"我们的出口"）
- `domain_words_trad`：19,934 繁体原文（系統控制臺/我們的出口）
- 引擎 `load_domain_trad_from_db`：trad 表并入 trad_index，繁体模式领域词出原生繁体
- 繁体模式候选转繁去重（"我們的出口" 原生 + 简体转繁重复 → 保留一个）
- taishen-dict `domains_db.py` 分集输出 + pipeline 4.2 只滤乱码保留繁体原文
...
## V0.5.7 common 表高频词重排——first_char_verify 10 缺词修复（2026-08-09）

**动机**：first_char_verify 测试 568 常用词中 10 个不在第一屏（前 5 候选不可达）：
- shi 使/事/始 缺失（实际 是/式/实/试/示）
- zhi 制/执/志/值 缺失（实际 只/知/置/直/之）
- ji 机/基/己 缺失（实际 计/记/级/辑/继）

### 根因

common 表三组词条超过第一屏容量 5，且行序与真实词频严重不符：
- shi 8 条：词频 4045 的使/3910 的事 被挤到 6-7 位
- zhi 9 条：超容量 4 条，低频置 反而排第 3
- ji 8 条：词频最低的辑(2359) 排第 4，机(3229) 排第 6

P2 层（common）按行序全量推入候选，同拼音 >5 词时前 5 位置被占满，
测试断言全部 common 词前 5 可达在数学上不可满足——必须按真实词频精简。

### 修复

按 system_dict.db 真实词频重排 + 每组保留 top5（超出词条由 system 层 P3 按词频兜底，不丢失）：

| 拼音 | 新 common 表（词频降序） | 移出（system 层兜底） |
|------|--------------------------|----------------------|
| shi | 是(4962)/使(4045)/事(3910)/式(3457)/实(3246) | 始/试/示 |
| zhi | 之(4329)/只(4209)/直(3533)/知(3508)/制(3422) | 置/值/志/执 |
| ji | 级(3358)/记(3256)/机(3229)/计(3107)/基(2994) | 继/己/辑 |

- common_dict.txt 558 条（-10），重建 common.db
- 实测候选序：shi 是/使/事/式/实，zhi 之/只/直/知/制，ji 级/记/机/计/基
- first_char_verify：558 常用词 0 缺词通过；common 相关单测 19 个全过

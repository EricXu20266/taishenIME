...
### shh 简拼缺"社会"（Eric 复测：选词逻辑三层问题）
- **问题**：① 词库缺"社会"词条（最常用词消失）② 即使补上，shh 简拼也排不进首屏 ③ 候选混入"社会化"等 3 字词。
- **根因链**（层层挖出）：
  1. **词库构建截取 bug**：build_dict.py `TOP_N_BASE=50000` 按 freq 降序截 base 词库，
     而雾凇 base 中 freq=9999（顶格常用词档）有 **82911 条**，截断点在 freq≥18020——
     所有 9999 档常用双字词（"社会"等）全被砍掉，仅靠 ext 词库兜底补回一部分，
     "社会"（只在 base 中）彻底消失。**5 万 → 12 万**（覆盖 9999 档全部），词库 62.2 万 → 69.2 万条。
  2. **简拼索引截断 bug**：`MAX_PREFIX_ENTRIES=100`——2 音节 sh+h 的同音异形词
     （上海/伤害/审核/身和…，freq 全为 9999 档 2739）超过 100 个，"社会"（shehui
     在拼音序中段）被截出 short_index_full["shh"]。**100 → 400**。
  3. **常用词层简拼缺失**：common_dict 的简拼索引用 compact 版（sh→s 归一），
     "社会"compact 为 "sh"≠"shh"，shh 时常用词层不命中，系统词"社会"被排到第 25 位。
     **新增 common_short_full_index**（zh/ch/sh 保留两字符），常用词在完整声母简拼下排最前。
- **验证**：`shh` → 社会/上海/时候/生活/伤害/审核/适合/是很/说话（社会第 1）。
  完整拼音 `shehui` → 社会第 1。shhh（3 音节）仍正确出"社会化"等。
- 覆盖：tools/build_dict.py / engine/src/dictionary/mod.rs / resources/common_dict.txt
  / system_dict.db（重建），commit de89539。

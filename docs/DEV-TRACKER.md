...
| 0.2.27 | 标点表对齐 rime half_shape：`@#%|~` 保留半角、`_`→——、`\`→顿号、数字分隔符 `,.:` | half_shape 预设 | 全角化过度（@＃％ 也全角了） | ⚪ 待排期 | P1 |
| 0.2.28 | 标点复选候选（`<` →《〈«‹）+ 配对引号（`'` `"` 自动开闭） | punctuator 多映射/pair | 无 | ⚪ 待排期 | P2 |
| 0.2.29 | 词库部署期预编译索引落盘，运行时秒加载（根治首次 6-7s） | 部署期编译 .bin，mmap 加载 | 运行期建索引（已幂等，首次仍慢） | ⚪ 待排期 | P1 |
